...
# get individual year data for LSOA21 with new time bands (night split into late evening and night)
...
add_daytime_evening_timeperiod <- function(bustrips, new_periods = TRUE) {

  if(new_periods) {
    bustrips <- bustrips %>%
      mutate(tph_weekday_dayevening = round(((tph_weekday_daytime * 12) + (tph_weekday_Evening * 6)) / 18, 2),
             tph_Sat_dayevening = round(((tph_Sat_daytime * 12) + (tph_Sat_Evening * 6)) / 18, 2),
             tph_Sun_dayevening = round(((tph_Sun_daytime * 12) + (tph_Sun_Evening * 6)) / 18, 2))
  } else {
    bustrips <- bustrips %>%
      mutate(tph_weekday_dayevening = round(((tph_weekday_daytime * 12) + (tph_weekday_Evening * 4)) / 16, 2),
             tph_Sat_dayevening = round(((tph_Sat_daytime * 12) + (tph_Sat_Evening * 4)) / 16, 2),
             tph_Sun_dayevening = round(((tph_Sun_daytime * 12) + (tph_Sun_Evening * 4)) / 16, 2))
  }


}

# lsoa bustrips = keep all source data

load_lsoa21_bustrips_alldays <- function(data_year = 2007) {
